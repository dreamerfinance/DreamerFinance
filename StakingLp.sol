...
contract StakingLp is ReentrancyGuard,Ownable {
    ...
    function stake(address lpToken, uint256 value) public nonReentrant {
        ...
        emit Stake(msg.sender,lpToken,value);
    }

    function getUser(address lpToken, address account)
        public
        view
        returns (
            uint256 stakes,
    ...
    {
        User storage user = _users[lpToken][account];
        Pool storage pool = _pools[lpToken];
        stakes = user.stakes;
        got = user.got;
    ...
    function reward(address lpToken) public nonReentrant {
    ...
    function redeem(address lpToken) public nonReentrant {
        User storage user = _users[lpToken][msg.sender];
        ...
        IERC20(lpToken).safeTransfer(msg.sender, user.stakes);
        emit Redeem(msg.sender,lpToken,user.stakes,_award,amount);
        delete _users[lpToken][msg.sender];
    }
